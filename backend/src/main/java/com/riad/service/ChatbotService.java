...
public class ChatbotService {
    ...
    private boolean matchesCapacity(String lower) {
        return lower
                .matches(".*(personne|person|capacité|capacity|personnes|guests|personas|أشخاص|شخص|\\d+\\s*pers).*");
    }
    ...
    private boolean matchesAmenities(String lower) {
        return lower.matches(
                ".*(équipement|amenity|amenities|service|wifi|piscine|pool|petit.déjeuner|breakfast|desayuno|مرافق|خدمات).*");
    }
    ...
    private ChatbotResponse handleGreeting(String lang) {
        return ChatbotResponse.text(switch (lang) {
            case "en" ->
                "Hello! Welcome to Riad Lee. 🌟 I'm your virtual assistant. I can help you with rooms, availability, prices, reservations, and information about Marrakech. How can I assist you today?";
            case "ar" ->
                "مرحباً! أهلاً وسهلاً في رياض لي. 🌟 أنا مساعدك الافتراضي. يمكنني مساعدتك في الغرف والتوافر والأسعار والحجوزات ومعلومات عن مراكش. كيف يمكنني مساعدتك اليوم؟";
            case "es" ->
                "¡Hola! Bienvenido al Riad Lee. 🌟 Soy tu asistente virtual. Puedo ayudarte con habitaciones, disponibilidad, precios, reservas e información sobre Marrakech. ¿Cómo puedo ayudarte hoy?";
            default ->
                "Bonjour ! Bienvenue au Riad Lee. 🌟 Je suis votre assistant virtuel. Je peux vous aider avec les chambres, la disponibilité, les tarifs, les réservations et des informations sur Marrakech. Comment puis-je vous aider ?";
        });
    }
    ...
    private ChatbotResponse handleAvailability(String lang) {
        ...
        if (available.isEmpty()) {
            return ChatbotResponse.text(switch (lang) {
                case "en" ->
                    "I'm sorry, there are currently no available rooms. Please check back later or contact us directly.";
                case "ar" -> "عذراً، لا توجد غرف متاحة حالياً. يرجى المراجعة لاحقاً أو التواصل معنا مباشرة.";
                case "es" ->
                    "Lo siento, actualmente no hay habitaciones disponibles. Por favor, vuelva más tarde o contáctenos directamente.";
                default ->
                    "Je suis désolé, il n'y a actuellement aucune chambre disponible. Veuillez réessayer plus tard ou nous contacter directement.";
            });
        }
    ...
    private ChatbotResponse handleRoomList(String lang) {
        ...
        if (rooms.isEmpty()) {
            return ChatbotResponse.text(switch (lang) {
                ...
                default ->
                    "Nous n'avons actuellement aucune chambre configurée. Contactez-nous pour plus d'informations.";
            });
        }
    ...
    private ChatbotResponse handlePrice(String lang) {
        ...
        String intro = switch (lang) {
            ...
            case "es" ->
                "Las tarifas de nuestras habitaciones oscilan entre " + priceRange + " por noche. Aquí los detalles:";
    ...
    private ChatbotResponse handleCapacity(String lower, String lang) {
        ...
        if (rooms.isEmpty()) {
            return ChatbotResponse.text(switch (lang) {
                case "en" -> "Unfortunately, we have no rooms that can accommodate " + guests
                        + " person(s). Please contact us for a customized solution.";
                case "ar" ->
                    "للأسف، لا توجد لدينا غرف تستوعب " + guests + " شخص/أشخاص. يرجى التواصل معنا للحصول على حل مخصص.";
                case "es" -> "Lamentablemente, no tenemos habitaciones para " + guests
                        + " persona(s). Contáctenos para una solución personalizada.";
                default -> "Malheureusement, nous n'avons pas de chambre pouvant accueillir " + guests
                        + " personne(s). Contactez-nous pour une solution personnalisée.";
            });
        }
    ...
    private String buildSystemPrompt(String lang) {
        ...
        return switch (lang) {
            case "en" ->
                "You are a helpful virtual receptionist for Riad Lee, a traditional Moroccan guesthouse in the Medina of Marrakech. Answer only public, safe questions about the riad, rooms, and Marrakech. Never share private, admin, or sensitive information. Rooms available:\n"
                        + roomContext;
            case "ar" ->
                "أنت موظف استقبال افتراضي مفيد لرياض لي، دار ضيافة مغربية تقليدية في المدينة القديمة بمراكش. أجب فقط على الأسئلة العامة والآمنة المتعلقة بالرياض والغرف ومراكش. لا تشارك أبداً معلومات خاصة أو إدارية أو حساسة. الغرف المتاحة:\n"
                        + roomContext;
            case "es" ->
                "Eres un recepcionista virtual útil del Riad Lee, una posada marroquí tradicional en la Medina de Marrakech. Responde solo preguntas públicas y seguras sobre el riad, las habitaciones y Marrakech. Nunca compartas información privada, administrativa o sensible. Habitaciones disponibles:\n"
                        + roomContext;
            default ->
                "Tu es un réceptionniste virtuel serviable pour le Riad Lee, une maison d'hôtes marocaine traditionnelle dans la Médina de Marrakech. Réponds uniquement aux questions publiques et sûres sur le riad, les chambres et Marrakech. Ne partage jamais d'informations privées, administratives ou sensibles. Chambres disponibles :\n"
                        + roomContext;
        };
    }
    ...
    private ChatbotRoomInfo toRoomInfo(Room room) {
        // Eagerly copy lazy collections into plain Java lists while the Hibernate
        // session is still open
        List<String> amenitiesCopy = List.copyOf(room.getAmenities());
